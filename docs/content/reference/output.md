...
describes what users get. Pick a format with `-o`, or let bc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
bc <command> -o table   # aligned columns for reading
bc <command> -o jsonl   # one JSON object per line, for piping
bc <command> -o json    # a single JSON array
bc <command> -o csv     # spreadsheet friendly
bc <command> -o tsv     # tab-separated
bc <command> -o url     # just the URL column
bc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
bc <command> --fields id,title,url
```
...
```bash
bc <command>            # a table, because this is a terminal
bc <command> | wc -l    # JSONL, because this is a pipe
```
